...
import { Home, Contacts, Portfolio } from "./pages";
    ...
    <div className="flex justify-center min-h-screen bg-[#121212] relative">
      <div className="w-[1200px] flex flex-col lg:flex-row space-y-4 lg:space-y-0 lg:justify-between p-4 pb-20 lg:py-16">
        ...
        <div className="bg-[#1e1e1f] lg:w-[70%] xl:w-[75%] border border-[#383838] rounded-3xl relative">
          {/* Top navigation */}
          <div className="hidden md:flex absolute top-0 right-0 flex-row space-x-10 bg-[#282829] border border-[#383838] p-6 px-14 rounded-bl-2xl rounded-tr-2xl text-white text-lg">
            {tabs.map((row, index) => (
              <div
                key={index}
                className={`${tab === row ? 'text-[#fcd16b]' : 'text-white'} cursor-pointer hover:text-[#fcd16b] transition-colors`}
                onClick={() => setTab(row)}
              >
                {row}
              </div>
